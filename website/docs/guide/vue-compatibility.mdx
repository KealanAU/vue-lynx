...
Vue's [`<Transition>`](https://vuejs.org/guide/built-ins/transition.html) and [`<TransitionGroup>`](https://vuejs.org/guide/built-ins/transition-group.html) components apply enter/leave animations when elements are inserted or removed.

`<Transition>` is **fully supported** — CSS transitions, CSS animations, `appear`, `mode`, JS hooks, and transitions with or without an explicit `:duration` prop all work. Since [#248](https://github.com/Huxpro/vue-lynx/issues/248) (v0.5), Vue Lynx also follows Vue's persisted transition behavior for `v-show`: enter and leave hooks run on every visibility change, and `display: none` is applied only after the leave animation finishes. Patterns such as drawers, sheets, and popovers behave consistently with official Vue on both Web and native Lynx.

Without `:duration`, Vue Lynx waits for the real `transitionend`/`animationend` event (same primary path as Vue DOM). A bounded fallback cleanup ([#286](https://github.com/Huxpro/vue-lynx/issues/286)) unregisters stranded handlers if a transition is interrupted before the event fires.

:::info Remaining limitation (`<TransitionGroup>` only)
Move (FLIP) animations in `<TransitionGroup>` are not supported — `getBoundingClientRect()` is unavailable from the background thread. Enter and leave transitions for list items still work.
:::

Scroll to **9. Registry cleanup** in the example below to see a no-`duration` `<Transition>` under rapid interruption: the event-registry counter climbs while hammering, then returns to baseline after stop.
...
|---------|--------|-------------|
| `<TransitionGroup>` move (FLIP) | `getBoundingClientRect()` is unavailable on the background thread | Enter/leave list transitions still work; animate layout changes with CSS or Main Thread Script if needed |
