...
Vue 的 [`<Transition>`](https://cn.vuejs.org/guide/built-ins/transition.html) 和 [`<TransitionGroup>`](https://cn.vuejs.org/guide/built-ins/transition-group.html) 组件在元素插入或移除时应用进入/离开动画。

`<Transition>` **已完整支持** —— CSS transition、CSS animation、`appear`、`mode`、JS hooks，以及带或不带显式 `:duration` prop 的过渡均可使用。从 [#248](https://github.com/Huxpro/vue-lynx/issues/248)（v0.5）开始，Vue Lynx 也遵循 Vue 对 `v-show` 持久化过渡的行为：每次可见性变化都会运行进入/离开钩子，并且只会在离场动画结束后应用 `display: none`。drawer、sheet、popover 等模式在 Web 和原生 Lynx 上均可与官方 Vue 表现一致。

不传 `:duration` 时，Vue Lynx 会等待真实的 `transitionend`/`animationend` 事件（与 Vue DOM 的主路径相同）。有界的回退清理（[#286](https://github.com/Huxpro/vue-lynx/issues/286)）会在过渡被打断、事件未到达时注销残留的处理器。

:::info 剩余限制（仅 `<TransitionGroup>`）
`<TransitionGroup>` 的移动（FLIP）动画尚不支持——后台线程无法使用 `getBoundingClientRect()`。列表项的进入/离开过渡仍然可用。
:::

在下面的示例中滚动到 **9. Registry cleanup**，可观察没有 `:duration` 的 `<Transition>` 在快速打断下的表现：hammer 时 event-registry 计数上升，停止后会回到 baseline。
...
| 特性 | 原因 | 替代方案 |
|---------|--------|-------------|
| `<TransitionGroup>` 移动（FLIP） | 后台线程无法使用 `getBoundingClientRect()` | 列表进入/离开过渡仍可用；布局变化可用 CSS 或主线程脚本实现 |
